...
main( int argc, char* argv[] )
{
  ...
  ifstream jsonFile ( "../topology/interface/dataDuke.txt", std::ios::in );       // Device - Node mapping
  ...
  if ( configFile.is_open () ) {
	...
	int mbps = std::stoi(netParams[2])*10;
	 ...
	 if ( IsLCAppInstalled( netParams[0] ) == false ) {
	    ...
	    // Install consumer
	    syncHelper.SetPrefix( "/power/typeI" );
	    ...
	    syncHelper.SetAttribute( "LifeTime", StringValue( "2s" ) );
	 ...
	 if ( IsPMUAppInstalled( netParams[1] ) == false ){
	    ...
	    for(int i = 0; i < nameMap[std::stoi( netParams[1] )].size(); i++ ) {
	       ...
	       if (found!=std::string::npos || com_string == "V" || com_string == "R" || com_string == "B"){
 	          pre = "/power/typeI";
		  continue;
	       }
	       if( (com_string == "P") && pre != "/power/typeI" )            
	          pre = "/power/typeII";
	 ...
	 flowfile << netParams[0] << " " << netParams[1] << " TypeIII" << std::endl;
	  ...
	  std::cout<< netParams[0] <<std::endl;
      ...
      } else if( injectData == true  && base == false) {
        netParams = SplitString( strLine );
	   char temp[10];
	   ...
	   syncHelper.SetPrefix( "/" );
	   syncHelper.SetAttribute( "Attacker", IntegerValue( 1 ) );
 	   //syncHelper.Install( nodes.Get( std::stoi( netParams[0] ) ) );
  ...
  tokenHelper.SetAttribute( "FillRates", StringValue( "300 500 1100 100" ) ); // 10 interests a second

  tokenHelper.SetAttribute( "Capacities", StringValue( "300 400 500 100" ) ); // 10 interests a second



  bool icaap = false;
   for ( int i=0; i<( int )nodes.size(); i++ ) {
    if( used[i] == 0 && !icaap and !base){
//	    if(false){
...
SentInterestCallbackPhy( uint32_t nodeid, shared_ptr<const ndn::Interest> interest ) {
        ...
        if ( interest->getName( ).getSubName( 0,1 ).toUri( ).substr( 0,4 ) == "/pow" ) 
	{
		return;
	}
	   ...
	   return;			        
        ...
        //If it is not update sync with arriving packets data
                ...
                if ( interest->getName().getSubName( 1,1 ).toUri( ).substr(0,5) == "/type"){
                        ...
                        bool lead = false;//sync.sendDirect( payload, nodeid,  interest->getName().getSubName( 3,1 ).toUri().substr( 1 ) );

                        if( !lead ) {
				//std::cout<< "\nFollower "<<str<<std::endl;
