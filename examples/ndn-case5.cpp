...
namespace ns3 {
    ...
    int main( int argc, char* argv[] )
    {
        ...
        if ( configFile.is_open () ) {
            while ( std::getline( configFile, strLine ) ) {
                ...
                if( gettingNodeCount == true ) {
                    ...
                    if ( IsPMUAppInstalled( netParams[1] ) == false ){ 
                        char temp[10];
                        sprintf( temp, "%lf", 1.0/( float( rand()%40+100 ) ) );         //========= AC -> LC (Type II - 120/sec) (100 - 140)
        ...
        tokenHelper.SetAttribute( "FillRate3", StringValue( "180" ) ); // 10 interests a second
        ...
        char trace[100];
        sprintf( trace, "ndn-case5-run%d.csv", run );
