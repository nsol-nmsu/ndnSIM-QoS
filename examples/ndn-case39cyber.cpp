...
main(int argc, char* argv[])
{
  ...
  ifstream configFile ("/home/george/folder/case.txt", std::ios::in);
  std::string strLine;
...
//  Config::SetDefault("ns3::DropTailQueue::MaxPackets", StringValue("10"));
 ...
 //ndn::StrategyChoiceHelper::InstallAll("/", "/localhost/nfd/strategy/qos");
  ...
  tokenHelper.SetAttribute("FillRate1", StringValue("4800")); // 10 interests a second
  tokenHelper.SetAttribute("Capacity1", StringValue("180")); // 10 interests a second
  tokenHelper.SetAttribute("FillRate2", StringValue("4700")); // 10 interests a second
  tokenHelper.SetAttribute("Capacity2", StringValue("170")); // 10 interests a second
  tokenHelper.SetAttribute("FillRate3", StringValue("1500")); // 10 interests a second
  tokenHelper.SetAttribute("Capacity3", StringValue("150")); // 10 interests a second

  for (int i=0; i<(int)nodes.size(); i++) {
	  if(used[i] == 0){
          ndn::StrategyChoiceHelper::Install(nodes.Get(i),"power/pdc/data", "/localhost/nfd/strategy/qos");
	  ndn::StrategyChoiceHelper::Install(nodes.Get(i),"power/wac/data", "/localhost/nfd/strategy/qos");
          ndn::StrategyChoiceHelper::Install(nodes.Get(i),"power/bgd", "/localhost/nfd/strategy/qos");
