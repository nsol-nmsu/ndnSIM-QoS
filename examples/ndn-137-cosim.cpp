...
main ( int argc, char* argv[] )
{
	...
	ifstream configFile ( "../topology/interface/Topo137-power.txt", std::ios::in );	// Topology file
	ifstream jsonFile ( "../topology/interface/Devices137-power.txt", std::ios::in );	// Device - Node mapping
		...
		while ( std::getline( configFile, strLine ) ) {
			...
			if ( gettingNodeCount == true ) {
				...
				if ( IsLCAppInstalled( netParams[0] ) == false ) {
					...
					auto apps = nodes.Get( std::stoi( netParams[0] ) )->GetApplication( 0 )->GetObject<ns3::ndn::ConsumerSync>();
