...
ParserOpenDSS::processJson( json njf, std::vector<std::string>* packetNames ) {
  ...
  while ( it !=  njf.end() ) {
    ...
    else if ( it.key() == "Switch" || it.key() == "Transformer" || it.key() == "Capacitor" || it.key() == "PVSystem" || it.key() == "Storage" || it.key() == "Load") {
      json::iterator it1 = it.value().begin();

      while ( it1 != it.value().end() ) {
        std::cout<<"Obtaining Device Name\n";
        consumer = ( *nameMap )[it1.key()];
        std::cout<<"Name: "<<it1.key()<<" Node: "<<consumer<<std::endl;
        device = it1.key();
...
ParserOpenDSS::processJson( json njf, std::vector<std::string>* packetNames ) {
  ...
  while ( it !=  njf.end() ) {
	...
	if (consumer == 0){
        ...
        //std::cout<<"Name: "<<it1.key()<<" Node: "<<consumer<<" Class:"<< it.key()<<std::endl;
        device = it1.key();
