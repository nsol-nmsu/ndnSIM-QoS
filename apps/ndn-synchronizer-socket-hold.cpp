...
#include <string.h>
#include <stdio.h>
#include "ndn-synchronizer-socket-hold.hpp"
#include "ns3/ptr.h"
#include "ns3/log.h"
...
void
SyncSocketHold::addArrivedPackets( std::string str ) {
	...
	//&& it.value()[updateInfo[1]].begin() != it.value()[updateInfo[1]].end()) {
        if(it.key() == "PVSystem" || it.key() == "Storage"){
		   ...
		   if(skip >= LOSS){
                      if (newDur == 0)
                         newDur = rand()%3 + 2;
		      DERs[nameMap[updateInfo[1]]] = newDur;
		   }
		} 
		if(DERs[nameMap[updateInfo[1]]] > 0){
			DERs[nameMap[updateInfo[1]]]--;
			return;
		}
...
bool
SyncSocketHold::sendDirect( std::string send_json, int src, std::string deviceName ) {
  ...
  while ( !arrivedPackets.empty() ) {
    ...
    if ( SendInfo[3] == "RedisPV" ) {
      ...
      if (duration == 0){
	     ...
	     if (newDur == 0)
                newDur = rand()%3 + 2;
             DERs[nameMap[SendInfo[1]]] = newDur;
          }
      }
       if(DERs[nameMap[SendInfo[1]]] > 0){
        DERs[nameMap[SendInfo[1]]]--;	
        //std::cout << "###################Skip packet: " << arrivedPackets.back() << "###############\n";
...
void
SyncSocketHold::sendData( std::string data, int socket ) {
...
std::string
SyncSocketHold::receiveData( int socket ) {
...
void
SyncSocketHold::processLeadJson( json jf, int src, std::string srcDevice ) {
...
void
SyncSocketHold::fillNameMap( json jf ) {
...
void
SyncSocketHold::initializeJson( json jf ){
...
void
SyncSocketHold::aggDER( std::string payload, int src, std::string follower, std::string lead ) {
...
void
SyncSocketHold::fillDERs( json jf ) {

  json::iterator itp = jf.begin();
